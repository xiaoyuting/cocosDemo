    ...
    execute: function () {
      ...
      _export("PhysicsManger", PhysicsManger = (_dec = ccclass('PhysicsManger'), _dec(_class = class PhysicsManger {
        ...
        currentPhyResult() {
          ...
          var results = PhysicsSystem2D.instance.raycast(v2(this.target.getWorldPosition().x, this.target.getWorldPosition().y), v2(this.target.getWorldPosition().x + (this.target.getComponent(UITransform).contentSize.width / 2 + this.distance) * dir.x), this.target.getWorldPosition().y + (this.target.getComponent(UITransform).contentSize.height / 2 + this.distance) * dir.y, ERaycast2DType.Closest);
